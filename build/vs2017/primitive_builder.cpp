...
void PrimitiveBuilder::Init()
{
	...
	default_plane_mesh_ = CreatePlaneMesh(gef::Vector4(5.0f, 0.0f, 5.0f), gef::Vector4(0.0f, 0.0f, 0.0f));
...
gef::Mesh* PrimitiveBuilder::CreateBoxMesh(const gef::Vector4& half_size, gef::Vector4 centre, gef::Material** materials)
{
	...
	// vertices
	//
	// create vertices, 4 for each face so we have all vertices in a single face share the same normal
...
// CreateBoxMesh
//
gef::Mesh* PrimitiveBuilder::CreatePlaneMesh(const gef::Vector4& half_size, gef::Vector4 centre, gef::Material* material)
{
	gef::Mesh* mesh = new gef::Mesh(platform_);
	...
	primitive->InitIndexBuffer(platform_, indices, kNumIndices, sizeof(Int32));
	primitive->set_type(gef::TRIANGLE_LIST);
	...
	// axis aligned bounding box
	gef::Vector4 min_vtx = centre - half_size;
	gef::Vector4 max_vtx = centre + half_size;
	min_vtx.set_y(0.0f);
	max_vtx.set_y(0.0f);

	gef::Aabb aabb(min_vtx, max_vtx);
	mesh->set_aabb(aabb);
